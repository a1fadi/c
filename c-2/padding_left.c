...
void pad_left(struct character_node *characters, char pad_character) {
    struct character_node *cur = characters;
    struct character_node *first = characters;
    struct character_node *one_behind = characters;
        first->data = pad_character;
        while (cur->next != NULL) {
        one_behind = cur->next;
        cur->next->data = one_behind->data;
        cur = cur->next;
        }
        cur = NULL;
